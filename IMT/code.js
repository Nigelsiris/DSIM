...
function setupConfigWeb() {
  try {
    setupConfigTab();
    return { success: true, message: 'Configuration tabs initialized successfully. Please review the 5 Configuration tabs.' };
  } catch(e) {
...
function getDashboardData() {
  ...
  const getRows  = (name) => { const s = getSheet(name); return s ? Math.max(0, s.getLastRow() - 1) : 0; };
  const sheetUrl = (name) => { const s = getSheet(name); return s ? ssUrl + '#gid=' + s.getSheetId() : null; };
  const embedUrl = (name) => { const s = getSheet(name); return s ? 'https://docs.google.com/spreadsheets/d/' + ssId + '/htmlembed?gid=' + s.getSheetId() + '&rm=minimal' : null; };

  // Row counts
  const masterRows      = getRows('Master Input');
  const tmstRows        = getRows('TMST');
  const discrepancies   = getRows('Discrepancy Tracker');
  const additionalCosts = getRows('Additonal Costs');

  // Config
  let config = {};
  try { config = getConfig(); } catch(e) {}

  // Primary carriers (those with _HAULIER_ID keys)
  const PRIMARY_CARRIERS = ['FRG','GRM','PYE'];

  // Build carrier objects
  const allCarrierKeys   = Object.keys(config).filter(k => k.endsWith('_ROOT_FOLDER'));
  const dynamicCarriers  = allCarrierKeys.map(k => k.replace('_ROOT_FOLDER', '')).filter(c => !PRIMARY_CARRIERS.includes(c));
  const allCarrierNames  = [...PRIMARY_CARRIERS, ...dynamicCarriers];

  const carriers = allCarrierNames.map(name => {
    const folderId  = config[name + '_ROOT_FOLDER'] || '';
    const haulierId = config[name + '_HAULIER_ID']  || '';
    return {
      name,
      classification: PRIMARY_CARRIERS.includes(name) ? 'Primary Lane' : 'Dynamic',
      primaryLane: PRIMARY_CARRIERS.includes(name),
  ...
  // Config tab readiness
  const CONFIG_TABS = ['System Config','GL Config','RDC Aliases','Email Template','Header Config'];
  const configReadyCount = CONFIG_TABS.filter(t => !!getSheet(t)).length;
  ...
  const discrepancyRate = masterRows > 0 ? Math.round((discrepancies / masterRows) * 100) : 0;
  const readinessScore  = Math.round(
    (configReadyCount / 5) * 40 +
    (configuredCarrierCount / Math.max(PRIMARY_CARRIERS.length, 1)) * 30 +
    (linkedHaulierCount / Math.max(PRIMARY_CARRIERS.length, 1)) * 30
  );

  const summary = { readinessScore, discrepancyRate, configReadyCount, configuredCarrierCount, primaryCarrierCount: PRIMARY_CARRIERS.length, linkedHaulierCount };

  // Alerts
  const alerts = [];
  if (configReadyCount < 5) alerts.push({ severity: 'warning', title: 'Configuration Incomplete', detail: configReadyCount + ' of 5 config tabs found. Run Initialization.' });
  if (configuredCarrierCount === 0) alerts.push({ severity: 'critical', title: 'No Carriers Configured', detail: 'Set root folder IDs in System Config before processing.' });
  if (discrepancyRate > 10) alerts.push({ severity: 'warning', title: 'High Discrepancy Rate', detail: discrepancyRate + '% of master rows have discrepancies.' });
  if (configReadyCount === 5 && configuredCarrierCount >= 1) alerts.push({ severity: 'success', title: 'System Ready', detail: 'All critical configuration present.' });

  // Operations sheets
  const OPS_SHEETS = [
    { name: 'Master Input',        label: 'Master Input',        description: 'Primary invoice line-item data extracted from carrier files' },
    { name: 'TMST',                label: 'TMST',                description: 'Transport Management System reconciliation data' },
    { name: 'Additonal Costs',     label: 'Additional Costs',    description: 'Non-base cost line items (tolls, fuel, NY pay, etc.)' },
    { name: 'Discrepancy Tracker', label: 'Discrepancy Tracker', description: 'Invoices flagged for discrepancies requiring resolution' }
  ];
  const operationsSheets = OPS_SHEETS.map(s => {
    const sheet = getSheet(s.name);
    return { ...s, rows: sheet ? Math.max(0, sheet.getLastRow() - 1) : 0, exists: !!sheet, openUrl: sheetUrl(s.name) };
  });

  // Config sheets
  const CFG_SHEETS = [
    { name: 'System Config',  label: 'System Config',  description: 'Folder IDs, haulier sheet IDs, and global settings' },
    { name: 'GL Config',      label: 'GL Config',      description: 'GL account mappings, cost centers, ignore rules, and tolerances' },
    { name: 'RDC Aliases',   label: 'RDC Aliases',    description: 'RDC code to name alias mappings for file classification' },
    { name: 'Email Template', label: 'Email Template', description: 'Discrepancy email subject and body templates' },
    { name: 'Header Config',  label: 'Header Config',  description: 'Column name alias mappings for dynamic header detection' }
  ];
  const configSheets = CFG_SHEETS.map(s => {
    const sheet = getSheet(s.name);
    return { ...s, rows: sheet ? Math.max(0, sheet.getLastRow() - 1) : 0, exists: !!sheet, openUrl: sheetUrl(s.name) };
  });

  // Viewer sheets (all ops + archive sheets)
  const viewerSheets = [];
  ss.getSheets().forEach(s => {
    const name = s.getName();
    const inOps    = OPS_SHEETS.some(o => o.name === name);
    const isArchive = name.startsWith('Archive -');
    if (inOps || isArchive) {
      const label = (OPS_SHEETS.find(o => o.name === name) || {}).label || name;
      viewerSheets.push({ name, label, rows: Math.max(0, s.getLastRow() - 1), embedUrl: embedUrl(name), openUrl: sheetUrl(name) });
    }
  });

  // Console seed
  const consoleSeed = [];
  consoleSeed.push({ message: 'Dashboard loaded. Workbook: ' + ssName, type: 'info' });
  if (configReadyCount < 5) consoleSeed.push({ message: 'WARNING: ' + (5 - configReadyCount) + ' config tab(s) missing.', type: 'warning' });
  if (configuredCarrierCount > 0) consoleSeed.push({ message: configuredCarrierCount + ' carrier lane(s) configured and ready.', type: 'success' });

  return {
    masterRows, tmstRows, discrepancies, additionalCosts,
    carriers, alerts, summary,
    operationsSheets, configSheets, viewerSheets,
    consoleSeed,
    workbook: { name: ssName, openUrl: ssUrl }
  };
}
...
function getConfigDataWeb() {
  ...
  return {
    systemConfig:  readSheet('System Config'),
    glConfig:      readSheet('GL Config'),
    rdcAliases:    readSheet('RDC Aliases'),
    emailTemplate: readSheet('Email Template'),
    headerConfig:  readSheet('Header Config')
  };
}

function saveSystemConfigWeb(rows) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let s = ss.getSheetByName('System Config');
    if (!s) s = ss.insertSheet('System Config');
    const headers = [['Setting Name','Value / ID','Description']];
    s.clearContents();
    s.getRange(1,1,1,3).setValues(headers).setFontWeight('bold').setBackground('#d9ead3');
    if (rows.length) s.getRange(2,1,rows.length,3).setValues(rows);
    return { success: true, message: 'System Config saved (' + rows.length + ' settings).' };
  } catch(e) { return { success: false, message: e.message }; }
}

function saveGlConfigWeb(rows) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let s = ss.getSheetByName('GL Config');
    if (!s) s = ss.insertSheet('GL Config');
    const headers = [['Rule Type','Keyword / RDC','GL Account / Center Code / Value','Description']];
    s.clearContents();
    s.getRange(1,1,1,4).setValues(headers).setFontWeight('bold').setBackground('#fff2cc');
    if (rows.length) s.getRange(2,1,rows.length,4).setValues(rows);
    return { success: true, message: 'GL Config saved (' + rows.length + ' rules).' };
  } catch(e) { return { success: false, message: e.message }; }
}

function saveRdcAliasesWeb(rows) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let s = ss.getSheetByName('RDC Aliases');
    if (!s) s = ss.insertSheet('RDC Aliases');
    const headers = [['RDC Code','Aliases (Comma Separated)']];
    s.clearContents();
    s.getRange(1,1,1,2).setValues(headers).setFontWeight('bold').setBackground('#cfe2f3');
    if (rows.length) s.getRange(2,1,rows.length,2).setValues(rows);
    return { success: true, message: 'RDC Aliases saved (' + rows.length + ' entries).' };
  } catch(e) { return { success: false, message: e.message }; }
}

function saveEmailTemplateWeb(rows) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let s = ss.getSheetByName('Email Template');
    if (!s) s = ss.insertSheet('Email Template');
    const headers = [['Setting','Template Text','Available Variables']];
    s.clearContents();
    s.getRange(1,1,1,3).setValues(headers).setFontWeight('bold').setBackground('#fce5cd');
    if (rows.length) s.getRange(2,1,rows.length,3).setValues(rows);
    return { success: true, message: 'Email Template saved.' };
  } catch(e) { return { success: false, message: e.message }; }
}

function saveHeaderConfigWeb(rows) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let s = ss.getSheetByName('Header Config');
    if (!s) s = ss.insertSheet('Header Config');
    const headers = [['Target Field','Column Name Aliases (Comma Separated)']];
    s.clearContents();
    s.getRange(1,1,1,2).setValues(headers).setFontWeight('bold').setBackground('#e6b8af');
    if (rows.length) s.getRange(2,1,rows.length,2).setValues(rows);
    return { success: true, message: 'Header Config saved (' + rows.length + ' field mappings).' };
  } catch(e) { return { success: false, message: e.message }; }
}